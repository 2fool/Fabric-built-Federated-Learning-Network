...
func main() {
	...
	// 连接到节点Web
	connectionWeb := newGrpcConnectionWeb()
	defer connectionWeb.Close()

	// 使用Web节点
	gatewayWeb, err := client.Connect(
		newIdentityWeb(),
		client.WithSign(newSignWeb()),
		client.WithClientConnection(connectionWeb),
	...
	defer gatewayWeb.Close()

	networkWeb := gatewayWeb.GetNetwork(channelName)
	contractWeb := networkWeb.GetContract(chaincodeName)
	// 调用CheckWorking方法，检查链码是否正常工作
	// checkWorking(contractWeb)

	// 上传参数
	uploadParameters(contractWeb, "web", getMockParameters(), 1)
}
