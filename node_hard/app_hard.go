...
func main() {
	...
	// 连接到节点Hard
	connectionHard := newGrpcConnectionHard()
	defer connectionHard.Close()

	// 使用Hard节点
	gatewayHard, err := client.Connect(
		newIdentityHard(),
		client.WithSign(newSignHard()),
		client.WithClientConnection(connectionHard),
	...
	defer gatewayHard.Close()

	networkHard := gatewayHard.GetNetwork(channelName)
	contractHard := networkHard.GetContract(chaincodeName)

	// 调用CheckWorking方法，检查链码是否正常工作
	// checkWorking(contractHard)

	// 上传参数
	uploadParameters(contractHard, "hard", getMockParameters(), 1)
}
